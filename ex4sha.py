...
def rotate_right(value, bits):
    """Circular right shift for 64-bit words."""
    return ((value >> bits) | (value << (64 - bits))) & 0xFFFFFFFFFFFFFFFF

def run_sha512():
    raw_input = input("Enter message: ")
    data = bytearray(raw_input.encode('utf-8'))
    bit_length = len(data) * 8

    # 1. Padding Logic
    # Append the '1' bit
    data.append(0x80)
    # Pad with zeros until we are 128 bits short of a 1024-bit multiple
    while (len(data) * 8) % 1024 != 896:
        data.append(0)
    # Append the original length as a 128-bit big-endian integer
    data += struct.pack('>QQ', 0, bit_length)

    # Initialize hash values
    current_hash = list(H_START)

    # 2. Process message in 128-byte (1024-bit) chunks
    for i in range(0, len(data), 128):
        ...
        for t in range(16, 80):
            ...
            s0 = rotate_right(w[t-15], 1) ^ rotate_right(w[t-15], 8) ^ (w[t-15] >> 7)
            s1 = rotate_right(w[t-2], 19) ^ rotate_right(w[t-2], 61) ^ (w[t-2] >> 6)
        ...
        for t in range(80):
            ...
            ch = (e & f) ^ ((~e) & g)
            temp1 = (h + S1 + ch + K_CONSTANTS[t] + w[t]) & 0xFFFFFFFFFFFFFFFF
        ...
        vars_after_round = [a, b, c, d, e, f, g, h]
        for j in range(8):
            current_hash[j] = (current_hash[j] + vars_after_round[j]) & 0xFFFFFFFFFFFFFFFF

    # Combine into final hex string
    result = "".join(f"{val:016x}" for val in current_hash)
    print(f"\nSHA-512 Hash: {result}")
